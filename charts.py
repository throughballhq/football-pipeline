...
def movers_bar(title: str, subtitle: str, movers: list, path: str,
    ...
    pad = max(0.01, max(abs(d) for d in deltas) * 0.08)
    for yi, d, c in zip(y, deltas, colors):
        # label INSIDE the bar tip - can never collide with axis names
        ax.text(d - pad if d >= 0 else d + pad, yi, f"{d:+.1f}",
                va="center", ha="right" if d >= 0 else "left",
                color=INK if c == LIME else WHITE,
